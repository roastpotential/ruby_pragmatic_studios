...
describe Treasure do
    ...
    before do
        @treasure1 = Treasure.new(:ration,   5.0)
        @treasure2 = Treasure.new(:bow,    25.0)
        @treasure3 = Treasure.new(:potion,  50.0)
        @treasure4 = Treasure.new(:gemstone,    100.0)
        @treasure5 = Treasure.new(:strange_armor,  200.0)
        @treasure6 = Treasure.new(:strange_weapon, 200.0)
        @treasure7 = Treasure.new(:rare_armor, 400.0)
        @treasure8 = Treasure.new(:rare_weapon, 400.0)
    end 
...
describe TreasureTrove do
    ...
    it "has a +1 armor, with a 200 value" do
        TreasureTrove::TREASURES[4].should == Treasure.new(:strange_armor, 200.0)
            # armor of the person's proficiency that gives them +1 to their AC; 200 gp
    end

    it "has a +1 weapon, with a 200 value" do
        TreasureTrove::TREASURES[5].should == Treasure.new(:strange_weapon,    200.0)
    ...
    it "has a +2 armor, with a 400 value" do
        TreasureTrove::TREASURES[6].should == Treasure.new(:rare_armor, 400.0)
            # armor of the person's proficiency that gives them +2 to their AC; 400 gp
    end

    it "has a +2 weapon worth 400 gp" do
        TreasureTrove::TREASURES[7].should == Treasure.new(:rare_weapon,    400.0)
